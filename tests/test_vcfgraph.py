...
from offsetbasedgraph.vcfgraph import construct_graph, VCFGraph, AdjList, SNPs, graph_from_snps_and_indels
...
def test_simple_insertion():
    graph = graph_from_snps_and_indels(np.array([[], []]), np.array([[2], [3]]), [], 10)
    true_graph = VCFGraph([3, 3, 7], AdjList([0, 2, 3, 3], [1, 2, 2]), SNPs())
    assert graph == true_graph


def test_two_insertion():
    graph = graph_from_snps_and_indels(np.array([[], []]), np.array([[2, 4], [3, 2]]),[], 10)
...
def test_simple_deletion():
    graph = graph_from_snps_and_indels(np.array([[3], [2]]), np.array([[], []]), [],  10)
...
def test_simple_snp():
    graph = graph_from_snps_and_indels(np.array([[], []]), np.array([[], []]), [2, 3, 5],  10)
...
def test_construct_graph():
    graph = construct_graph(get_vcf_entries(path), 100000000)
    print(graph)
