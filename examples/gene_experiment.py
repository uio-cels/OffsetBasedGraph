...
def analyse_multipath_genes2(args):
    ...
    for b in ["chr13_KI270838v1_alt"]:
        if "alt" in b:
            print()
            print("Analysing genes on alt locus %s (number %d of %d)" % (
                b, n_a, len([bl for bl in text_graph.blocks if "alt" in bl])))
            n_a += 1
            ...
            subgraph = complex_graph.create_subgraph_from_intervals(
                subgraph_intervals,
                30000, b)
            ...
            print("  Equal: %d, equal exons: %d, total %d genes" % (
                equal, equal_exons, len(genes_here)))
